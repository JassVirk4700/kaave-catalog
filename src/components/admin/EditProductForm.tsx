...
import { FiUploadCloud, FiX, FiPlus, FiCheck, FiLoader, FiArrowLeft } from "react-icons/fi";
...
export default function EditProductForm({ initialData }: EditProductFormProps) {
  ...
  const [workType, setWorkType] = useState(initialData.work_type || "");
  ...
  useEffect(() => {
    if (initialData.image_urls) {
      setImages(
        initialData.image_urls.map((url: string) => ({
          preview: url,
          publicUrl: url,
          isExisting: true,
        }))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const folderPath = `${categoryConfig?.slug}/${subcategoryConfig?.slug}/${initialData.slug}`; // Keep original slug for storage path consistency unless renamed

      // 1. Upload only new images
      for (let i = 0; i < images.length; i++) {
        const img = images[i];
        
        if (img.isExisting && img.publicUrl) {
          finalImageUrls.push(img.publicUrl);
          continue;
        }
        ...
        if (img.file) {
          ...
          const filePath = `${folderPath}/added_${timestamp}_${i}.${ext}`;
          ...
          setImages((prev) =>
            prev.map((item, idx) =>
              idx === i ? { ...item, uploading: false, publicUrl: urlData.publicUrl, isExisting: true } : item
            )
          );
        }
      }
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          name: name.trim(),
          slug: toSlug(name), // Allow slug to update if name changes, but storage stays consistent via initialData.slug for now or we migrate storage (complex)
          category,
          subcategory,
    ...
    <div className="min-h-screen bg-[#fcfaf7] font-sans">
      <main className="w-full px-4 sm:px-6 lg:px-8 py-8 max-w-4xl mx-auto">
        ...
        <div className="mb-8 border-b border-[#eae6df] pb-6 flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2 text-[#873d3d] mb-2">
               <button onClick={() => router.back()} className="hover:bg-[#873d3d]/5 px-2 py-1 rounded-lg transition-colors flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider">
                 <FiArrowLeft size={14} /> Back
               </button>
            </div>
            <h1 className="text-3xl font-serif font-bold text-gray-800 tracking-tight">Edit Product</h1>
            <p className="text-gray-500 mt-1 text-sm font-light">
              Modifying: <span className="font-medium text-[#873d3d]">{initialData.name}</span>
            </p>
          </div>
          <div className="hidden md:block">
             <span className="text-[10px] font-bold uppercase tracking-widest text-gray-400 bg-white border border-[#eae6df] px-3 py-1.5 rounded-full shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="md:col-span-2">
                ...
                <p className="text-xs text-gray-400 mt-1.5 font-mono">
                  New Slug Preview: <span className="text-[#873d3d] font-semibold">{slug}</span>
                </p>
              </div>
            ...
            <div className="space-y-2.5">
              {specs.map((spec, i) => (
                <div key={i} className="flex gap-2 items-center">
                  <span className="text-[#873d3d] font-bold text-lg mt-0.5">•</span>
                  <input
                    type="text"
                    value={spec}
              ...
              <button type="button" onClick={addSpec} className="flex items-center gap-1.5 text-xs font-bold text-[#873d3d] hover:text-[#722f2f] mt-2 transition">
          ...
          {/* ─── SECTION: Images ──────────────────────────────────────────────── */}
          <section className="bg-white border border-[#eae6df] rounded-2xl p-6 shadow-sm">
            <h2 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-4">Product Images</h2>
            ...
            >
              ...
              <p className="text-sm font-medium text-gray-500">Drag & drop new images or <span className="text-[#873d3d] font-bold">click to browse</span></p>
              ...
              <div className="mt-4 grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
                {images.map((img, i) => (
                  <div key={i} className="relative group aspect-square rounded-xl overflow-hidden border border-[#eae6df] bg-[#f5f3f0]">
                    <Image src={img.preview} alt={`Preview ${i + 1}`} fill className="object-cover" sizes="100px" />
                    {img.uploading && (
                      <div className="absolute inset-0 bg-white/70 flex items-center justify-center">
                        <FiLoader size={16} className="animate-spin text-[#873d3d]" />
                      </div>
                    )}
                    {img.isExisting && !img.uploading && (
                      <div className="absolute top-1 left-1 px-1.5 py-0.5 bg-green-500 text-white text-[8px] font-bold uppercase rounded shadow-sm">Live</div>
                    )}
                    <button
                      type="button"
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-black/50 text-white text-[9px] text-center py-0.5 font-bold">{i + 1}</div>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="flex gap-3 pt-2 pb-6">
            ...
            <button
              ...
              className={`flex-1 py-3.5 rounded-xl font-bold tracking-wide text-sm flex items-center justify-center gap-2 transition-all shadow-md
                ${success ? "bg-green-600 text-white" : "bg-[#873d3d] hover:bg-[#722f2f] text-white disabled:opacity-60 disabled:cursor-not-allowed"}
              `}
            >
              {submitting ? (
                <><FiLoader className="animate-spin" size={16} /> Saving Changes…</>
              ) : success ? (
                <><FiCheck size={16} /> Changes Saved! Redirecting…</>
              ) : (
                "Save Changes"
              )}
            </button>
